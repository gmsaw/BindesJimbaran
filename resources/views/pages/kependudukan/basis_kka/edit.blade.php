              ...
              <div class="p-4 bg-gray-50">
                ...
                <form action="{{ route('kependudukan.update', ['npk' => $kk_adat->npk]) }}" method="POST">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div><label class="block text-sm font-medium text-gray-700 mb-1">Status Keanggotaan</label><select name="status_keanggotaan" class="w-full px-3 py-2 border border-gray-300 rounded-md">@foreach(['aktif', 'pindah_kk_sama_banjar', 'pindah_kk_beda_banjar', 'keluar_adat', 'meninggal', 'lainnya'] as $val)<option value="{{$val}}" {{ old('status_keanggotaan', $anggota_to_edit->status_keanggotaan) == $val ? 'selected' : '' }}>{{$val}}</option>@endforeach</select></div>
