    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        ...
        <div class="mt-4 sm:mt-0">
          <a href="{{ route('kependudukan.index', ['krama_request' => $krama_request]) }}" class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-300 rounded-lg shadow-sm">
      ...
      <!-- Form Ubah Keterangan Keluarga (Dadia, Penatahan, dll) -->
      <div class="bg-white p-6 rounded-xl shadow-lg mb-8">
        <h2 class="text-lg font-semibold text-gray-800 mb-4 border-b pb-2">
          <i class="fas fa-book-open mr-2 text-blue-500"></i>Keterangan Keluarga
        </h2>
        <form action="{{ route('kependudukan.update', ['npk' => $kk_adat->npk]) }}" method="POST">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label for="id_dadia_penatahan_fk" class="block text-sm font-medium text-gray-700 mb-1">Dadia / Penatahan</label>
              <select name="id_dadia_penatahan_fk" id="id_dadia_penatahan_fk" class="w-full px-3 py-2 border border-gray-300 rounded-md">
                <option value="">-- Tidak Ada --</option>
                @foreach ($all_dadia as $dadia)
                  <option value="{{ $dadia->id }}" {{ old('id_dadia_penatahan_fk', $kk_adat->keteranganKeluarga->id_dadia_penatahan_fk ?? '') == $dadia->id ? 'selected' : '' }}>
                    {{ $dadia->nama_dadia }} {{ $dadia->nama_penatahan ? '('.$dadia->nama_penatahan.')' : '' }}
                  </option>
                @endforeach
              </select>
            </div>
          ...
          <div class="flex justify-end mt-4">
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition">
              <i class="fas fa-save mr-1"></i> Simpan Keterangan
            </button>
          </div>
        </form>
      </div>
      ...
      <div class="bg-white p-6 rounded-xl shadow-lg mb-8">
        <h2 class="text-lg font-semibold text-gray-800 mb-4 border-b pb-2">
          <i class="fas fa-user-shield mr-2 text-blue-500"></i>Ubah Kepala Keluarga (Pengarep)
        </h2>
        <form action="{{ route('kependudukan.update', ['npk' => $kk_adat->npk]) }}" method="POST">
          @csrf
          @method('PUT')
          ...
          <div class="flex items-end gap-4">
            ...
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition h-fit">
              <i class="fas fa-save mr-1"></i> Ganti Kepala Keluarga
            </button>
          </div>
        </form>
      </div>
      ...
      <div class="space-y-4">
        <h2 class="text-lg font-semibold text-gray-800 mb-2">
          <i class="fas fa-user-friends mr-2 text-blue-500"></i> Edit Anggota Keluarga
        </h2>
        @foreach ($anggota_list as $anggota)
          <div class="border border-gray-200 rounded-lg overflow-hidden">
            ...
            <div class="bg-gray-100 px-4 py-3 flex justify-between items-center">
              <h3 class="font-medium text-gray-800">
                ...
                @if($anggota->status_hubungan_adat == 'kepala_keluarga')
                  <span class="ml-2 text-xs font-semibold text-blue-800 bg-blue-100 px-2 py-0.5 rounded-full">Kepala Keluarga</span>
                @endif
              </h3>
              <a href="{{ route('kependudukan.edit', ['npk' => $kk_adat->npk, 'edit_anggota' => $anggota->id_keanggotaan]) }}"
                 class="px-4 py-1 bg-white border border-gray-300 text-sm rounded-md hover:bg-gray-50 transition">
                <i class="fas fa-edit mr-1"></i> Edit
              </a>
            </div>

            <!-- Form Edit (Hanya tampil jika ID anggota cocok dengan parameter URL) -->
