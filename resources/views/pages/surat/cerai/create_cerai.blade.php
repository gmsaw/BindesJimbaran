...
@section('maincontent')
  <div class="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50">
    <div class="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <form action="{{ route('surat.cerai.store') }}" method="POST">
        @csrf
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          ...
          <div class="mt-4 sm:mt-0 flex gap-x-2">
            ...
            <a href="{{ route('surat.cerai.penomoran.index') }}" class="bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 px-4 rounded-lg shadow-sm"><i class="fas fa-database mr-2"></i>Penomoran</a>
            <a href="{{ route('surat.arsip.cerai') }}" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg shadow-sm"><i class="fas fa-database mr-2"></i>Arsip</a>
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg mb-8">
          ...
          <div class="mb-4">
            ...
            <div class="flex items-center space-x-4">
              <label class="flex items-center">
                ...
                onchange="window.location.href = '{{ route('surat.cerai.create') }}?penomoran=otomatis'">
                <span class="ml-2">Otomatis</span>
              </label>
              <label class="flex items-center">
                <input type="radio" name="penomoran" value="manual" {{ request('penomoran') == 'manual' ? 'checked' : '' }}
                onchange="window.location.href = '{{ route('surat.cerai.create') }}?penomoran=manual'">
          ...
          @if(request('penomoran', 'otomatis') == 'otomatis')
            <div class="bg-gray-50 p-4 rounded-md border">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                ...
                <button type="submit" formaction="{{ route('surat.cerai.create') }}" formmethod="GET" class="px-4 py-2 bg-gray-200 rounded-md text-sm">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="block text-sm font-medium">Tanggal Cerai</label><input type="date" name="tanggal_cerai" value="{{ old('tanggal_cerai', \Carbon\Carbon::now()->toDateString()) }}" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md"></div>
          ...
          <div class="bg-white p-6 rounded-xl shadow-lg">
            ...
            <div class="mb-4">
              ...
              <div class="flex gap-x-2">
                ...
                <button type="submit" formaction="{{ route('surat.cerai.create') }}" formmethod="GET" class="mt-1 px-4 py-2 bg-gray-200 rounded-md">Cari</button>
              </div>
            </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg mt-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- Kolom Kiri: Pemuput Karya -->
            {{-- Menggunakan Alpine.js untuk kontrol UI --}}
            <div x-data="{ pemuputAktif: {{ old('pemuput_toggle') === 'yes' ? 'true' : 'false' }} }">

              {{-- Label dengan checkbox di dalamnya --}}
              <label for="pemuput_karya" class="flex items-center text-sm font-medium text-gray-700">
                <input type="checkbox" name="pemuput_toggle" value="yes" x-model="pemuputAktif" class="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 mr-2">
                <span>Nama Pemuput Karya</span>
              </label>

              {{-- Input text yang dikontrol oleh checkbox --}}
              <input type="text" name="pemuput_karya" id="pemuput_karya"
                     value="{{ old('pemuput_karya') }}"
                     :disabled="!pemuputAktif"
                     :class="{ 'bg-gray-200 cursor-not-allowed': !pemuputAktif }"
                     class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md transition">
            </div>
            ...
            <div><label class="block text-sm font-medium">Kepala Lingkungan ...</label><input type="text" name="kepala_lingkungan" value="{{ old('kepala_lingkungan') }}" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md"></div>
            <div><label class="block text-sm font-medium">Kelihan Banjar ...</label><input type="text" name="kelihan_adat_banjar" value="{{ old('kelihan_adat_banjar') }}" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md"></div>

            <div><label class="block text-sm font-medium">Lurah</label><input type="text" name="lurah" value="{{ old('lurah') }}" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md"></div>
            <div><label class="block text-sm font-medium">Camat</label><input type="text" name="camat" value="{{ old('camat') }}" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md"></div>
          </div>
        </div>
      </form>
    </div>
