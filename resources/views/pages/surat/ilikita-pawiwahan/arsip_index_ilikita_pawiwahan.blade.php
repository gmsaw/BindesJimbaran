    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <div class="w-full md:w-auto flex flex-col md:flex-row gap-3">
          <a href="{{ route('surat.ilikita_pawiwahan.create') }}" class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-300 rounded-lg shadow-sm">
            <i class="fas fa-arrow-left mr-2"></i>Kembali
          </a>
          <form method="GET" action="{{ route('surat.arsip.ilikita_pawiwahan') }}" class="flex flex-1">
            ...
            @if($searchTerm)
              <a href="{{ route('surat.arsip.ilikita_pawiwahan') }}" class="ml-2 bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-md flex items-center"><i class="fas fa-times mr-1"></i> Reset</a>
            @endif
          </form>
        </div>
      </div>
            ...
            @forelse($arsip_surat as $surat)
              <tr>
                ...
                <td class="px-4 py-3 whitespace-nowrap text-right">
                  <a href="{{ route('surat.ilikita_pawiwahan.preview', $surat->id) }}" target="_blank" class="text-blue-600 hover:text-blue-900 p-1" title="Lihat & Cetak Surat">
                    <i class="fas fa-print"></i> Lihat
                  </a>

                  <a href="{{ route('surat.ilikita_pawiwahan.delete', $surat->id) }}" class="text-red-600 hover:text-red-900 p-1" title="Hapus Surat">
