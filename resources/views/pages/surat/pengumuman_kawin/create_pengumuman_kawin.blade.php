...
@section('maincontent')
  <div class="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50">
    <div class="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <form action="{{ route('surat.pengumuman_kawin.store') }}" method="POST">
        @csrf
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
          ...
          <div class="mt-4 sm:mt-0 flex gap-x-2">
            ...
            <a href="{{ route('surat.pengumuman.penomoran.index') }}" class="bg-yellow-600 hover:bg-yellow-700 text-white font-semibold py-2 px-4 rounded-lg shadow-sm"><i class="fas fa-database mr-2"></i>Penomoran</a>
            <a href="{{ route('surat.arsip.pengumuman') }}" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg shadow-sm"><i class="fas fa-database mr-2"></i>Arsip</a>
        ...
        <div class="bg-white p-6 rounded-xl shadow-lg mb-8">
          ...
          <div class="mb-4">
            ...
            <div class="flex items-center space-x-4">
              <label class="flex items-center">
                ...
                onchange="window.location.href = '{{ route('surat.pengumuman_kawin.create') }}?penomoran=otomatis'">
                <span class="ml-2">Otomatis</span>
              </label>
              <label class="flex items-center">
                <input type="radio" name="penomoran" value="manual" {{ request('penomoran') == 'manual' ? 'checked' : '' }}
                onchange="window.location.href = '{{ route('surat.pengumuman_kawin.create') }}?penomoran=manual'">
          ...
          @if(request('penomoran', 'otomatis') == 'otomatis')
            <div class="bg-gray-50 p-4 rounded-md border">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                ...
                <button type="submit" formaction="{{ route('surat.pengumuman_kawin.create') }}" formmethod="GET" class="px-4 py-2 bg-gray-200 rounded-md text-sm">
          ...
          @if(request('penomoran') == 'manual')
            <div class="mt-4">
              ...
              <input type="text" name="nomor_surat_manual" value="{{ old('nomor_surat_manual') }}" placeholder="Contoh: 001/IW-DAJ/2025" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md">
            </div>
          @endif
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label class="block text-sm font-medium">Hari Surat</label><input type="text" name="hari_surat" value="{{ old('hari_surat', \Carbon\Carbon::now()->translatedFormat('l')) }}" class="mt-1 w-full px-3 py-2 border border-gray-300 rounded-md"></div>
          ...
          <div class="bg-white p-6 rounded-xl shadow-lg">
            ...
            <div class="mb-4">
              ...
              <div class="flex gap-x-2">
                ...
                <button type="submit" formaction="{{ route('surat.pengumuman_kawin.create') }}" formmethod="GET" class="mt-1 px-4 py-2 bg-gray-200 rounded-md">Cari</button>
              </div>
            </div>
