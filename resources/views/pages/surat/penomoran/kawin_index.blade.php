  ...
  <div class="flex-1 overflow-y-auto p-4 sm:p-6 bg-gray-50">
    <div class="container mx-auto px-4 py-8 max-w-5xl">
      ...
      <div class="bg-white p-6 rounded-xl shadow-lg mb-8">
        ...
        <form action="{{ route('surat.kawin.penomoran.store') }}" method="POST">
          @csrf
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            @forelse($counters as $counter)
              <tr>
                ...
                <td class="px-4 py-3">
                  <form action="{{ route('surat.kawin.penomoran.update') }}" method="POST">
                    @csrf
                    @method('PUT')
                ...
                <td class="px-4 py-3 whitespace-nowrap text-right">
                  <form action="{{ route('surat.kawin.penomoran.destroy') }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kode surat ini?');">
                    @csrf
                    @method('DELETE')
                    <input type="hidden" name="kode_surat" value="{{ $counter->kode_surat }}">
                    <input type="hidden" name="tahun" value="{{ $counter->tahun }}">
