    ...
    <div class="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <div>
          ...
          <p class="text-gray-600">Tambah, edit, atau hapus kode dan nomor urut untuk kategori Surat Pengumuman.</p>
        </div>
        <div class="mt-4 sm:mt-0">
          <a href="{{ route('surat.pengumuman_kawin.create') }}" class="bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-300 rounded-lg shadow-sm">
      ...
      <div class="bg-white p-6 rounded-xl shadow-lg mb-8">
        ...
        {{-- Form ini sekarang mengarah ke route yang spesifik --}}
        <form action="{{ route('surat.pengumuman.penomoran.store') }}" method="POST">
          @csrf
          ...
          <table class="min-w-full divide-y divide-gray-200">
            ...
            @forelse($counters as $counter)
              <tr>
                ...
                <td class="px-4 py-3">
                  <form action="{{ route('surat.pengumuman.penomoran.update') }}" method="POST">
                    @csrf
                    @method('PUT')
                ...
                <td class="px-4 py-3 whitespace-nowrap text-right">
                  <form action="{{ route('surat.pengumuman.penomoran.destroy') }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kode surat ini?');">
                    @csrf
                    @method('DELETE')
                    <input type="hidden" name="kode_surat" value="{{ $counter->kode_surat }}">
                    <input type="hidden" name="tahun" value="{{ $counter->tahun }}">
            ...
            @empty
              <tr><td colspan="4" class="px-4 py-4 text-center text-gray-500">Belum ada data penomoran.</td></tr>
            @endforelse
            </tbody>
          </table>
        </div>
